...
16x16 RGBA pixel texture with nearest filtering, a real one-joint skin with no
more than four weights per vertex, and `idle`, `walk`, `hurt`, and `death`
animation clips. The simple proportions and palettes are intentionally distinct
for each creature and are inputs in the generator's `MODELS` table.
